...
Le lecteur d'espaces de travail est maison, limité au champ `packages` (séquence en bloc, séquence de flux sur une ligne ou plusieurs, fins de ligne Unix comme Windows, commentaires de fin de ligne compris) : tout ce qu'il ne sait pas lire désactive la détection, aucun dossier n'est écarté, et le résumé dit pourquoi.
Deux formes sont lues sans être appliquées, toujours du côté qui protège un dossier de trop : une négation (`!**/test/**`) et un alias YAML ne matchent aucun dossier, donc un sous-projet que pnpm retire de l'espace de travail reste mesuré au lieu d'être écarté.
